...
```
Before explaining the difference between a probability and a likelihood, let's first check visually.

Let's say we have data from a normal distribution with mean = 30 and standard deviation = 1.

The **probability** of getting less than the observed value 28 given the distribution is:

$P(X<=28~|~N(30, 1))$ which is the area under the curve below 28 as shown in the graph.
...
The **likelihood** of a distribution with mean = 30 and standard deviation = 1 given an observation = 28 is the y-axis value of our normal probability density function from the observed data point. Mathematically:

$L(N(30, 1)~|~X=28)$ We can get the y-axis value using `dnorm`
